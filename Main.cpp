...
int main(void) {
    ...
    if (blnKNNTrainingSuccessful == true) {                            // if KNN training was not successful
                                                                        // show error message
    ...
    imgOriginalScene = cv::imread("IMG-20171213-WA0007.jpg");         // open image
    ...
    if (vectorOfPossiblePlates.empty()) {                                               // if no plates were found
        ...
        std::cout << std::endl << "Numar citit = " << licPlate.strChars << std::endl;     // write license plate text to std out
        std::cout << std::endl << "-----------------------------------------" << std::endl;
		std::string Wlist[20] = { "VN97MHL","CT05MGO","CT27CZM",
			"GR27RBY","B770MAT","B379MIH","AG92ANP","B68XGL" };
		std::string Blist[20] = { "CT88SRT","IL04JFO","B55FXS",
			"CL22GYA","PH70DCA","CT77RTY","B46YCW","CT11HSC" };
		int OK = 0;
		for (int i = 0; i < size(Wlist); i++)
		{
			if (licPlate.strChars._Equal(Wlist[i]))
			{
				std::cout << "Permisa intrarea";
				OK = 1;
				break;
			}
			else
				if (licPlate.strChars._Equal(Blist[i]))
				{
					std::cout << "Intrare interzisa";
				OK = 1;
					break;
				}
		...
		if (OK == 0)
			std::cout << "Numar neinregistrat, acces interzis";
